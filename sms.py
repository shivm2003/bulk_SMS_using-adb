...
def adb_run(*args, retries=2):
    """Run an ADB command targeting the physical device. Retries on failure."""
    cmd = [ADB, "-d"] + list(args)
...
# Found via uiautomator dump: bounds="[888,2022][1056,2141]" -> center (972, 2081)
SEND_TAP_X = 972
SEND_TAP_Y = 2081

def send_sms(number, message):
    """
    Sends an SMS using ADB + Google Messages:
    1. Opens SMS app with the number and pre-filled message
    2. Directly taps the Send button at known coordinates
    """
    print(f"Sending SMS to {number} via ADB...")
    try:
